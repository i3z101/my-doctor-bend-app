...
const validatePasswordOnChange = (value, name, min, max) => {
    let message = "";
    if(value.trim() == ""){
        message = `${name} required`;
    }else if(!value.trim().match(/^[A-Za-z0-9!@#$%^&*]+$/)){
        message = `${name} should contain english and number and symbols characters(s)`;
    }else if(value.length < min || value.length > max) {
        message = `${name} must be between ${min} and ${max}`;
    }else {
        message = ""
